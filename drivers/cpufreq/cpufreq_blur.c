/*
 * drivers/cpufreq/cpufreq_blur.c
 *
...
#include <trace/events/cpufreq_blur.h>

static int active_count;

struct cpufreq_blur_cpuinfo {
...
static DEFINE_PER_CPU(struct cpufreq_blur_cpuinfo, cpuinfo);
...
static int cpufreq_governor_blur(struct cpufreq_policy *policy,
		unsigned int event);

#ifndef CONFIG_CPU_FREQ_DEFAULT_GOV_BLUR
static
#endif
struct cpufreq_governor cpufreq_gov_blur = {
	.name = "blur",
	.governor = cpufreq_governor_blur,
...
static void cpufreq_blur_timer_resched(
	struct cpufreq_blur_cpuinfo *pcpu)
{
 ...
 * function.
 */
static void cpufreq_blur_timer_start(int cpu)
{
	struct cpufreq_blur_cpuinfo *pcpu = &per_cpu(cpuinfo, cpu);
...
static unsigned int choose_freq(
	struct cpufreq_blur_cpuinfo *pcpu, unsigned int loadadjfreq)
{
...
static void cpufreq_blur_timer(unsigned long data)
{
	u64 now;
	unsigned int delta_time;
	u64 cputime_speedadj;
	...
	struct cpufreq_blur_cpuinfo *picpu;
	...
	if (pcpu->target_freq == new_freq) {
		trace_cpufreq_blur_already(
			data, cpu_load, pcpu->target_freq,
			pcpu->policy->cur, new_freq);
		goto rearm_if_notmax;
	}

	trace_cpufreq_blur_target(data, cpu_load, pcpu->target_freq,
					 pcpu->policy->cur, new_freq);
...
rearm:
	if (!timer_pending(&pcpu->cpu_timer))
		cpufreq_blur_timer_resched(pcpu);
...
static void cpufreq_blur_idle_start(void)
{
	struct cpufreq_blur_cpuinfo *pcpu =
...
static void cpufreq_blur_idle_end(void)
{
	struct cpufreq_blur_cpuinfo *pcpu =
		&per_cpu(cpuinfo, smp_processor_id());
	...
	if (!timer_pending(&pcpu->cpu_timer)) {
		...
		cpufreq_blur_timer(smp_processor_id());
	}

	up_read(&pcpu->enable_sem);
}

static int cpufreq_blur_speedchange_task(void *data)
{
	unsigned int cpu;
	cpumask_t tmp_mask;
	unsigned long flags;
	...
	while (1) {
		...
		for_each_cpu(cpu, &tmp_mask) {
			...
			for_each_cpu(j, pcpu->policy->cpus) {
				struct cpufreq_blur_cpuinfo *pjcpu =
					&per_cpu(cpuinfo, j);
			...
			if (max_freq != pcpu->policy->cur)
				__cpufreq_driver_target(pcpu->policy,
							max_freq,
							CPUFREQ_RELATION_H);
			trace_cpufreq_blur_setspeed(cpu,
						     pcpu->target_freq,
						     pcpu->policy->cur);
...
static void cpufreq_blur_boost(void)
{
	int i;
	int anyboost = 0;
	unsigned long flags;
...
static int cpufreq_blur_notifier(
...
static struct notifier_block cpufreq_notifier_block = {
	.notifier_call = cpufreq_blur_notifier,
};
			   ...
			   const char *buf, size_t count)
{
	...
	if (boost_val) {
		trace_cpufreq_blur_boost("on");
		cpufreq_blur_boost();
	} else {
		trace_cpufreq_blur_unboost("off");
	}
				...
				const char *buf, size_t count)
{
	...
	trace_cpufreq_blur_boost("pulse");
	cpufreq_blur_boost();
	return count;
}
...
static struct attribute *blur_attributes[] = {
...
static struct attribute_group blur_attr_group = {
	.attrs = blur_attributes,
	.name = "blur",
};

static int cpufreq_blur_idle_notifier(struct notifier_block *nb,
					     unsigned long val,
					     void *data)
{
	switch (val) {
	case IDLE_START:
		cpufreq_blur_idle_start();
		break;
	case IDLE_END:
		cpufreq_blur_idle_end();
		break;
	}
...
static struct notifier_block cpufreq_blur_idle_nb = {
	.notifier_call = cpufreq_blur_idle_notifier,
};
		...
		unsigned int event)
{
	...
	case CPUFREQ_GOV_START:
		...
		idle_notifier_register(&cpufreq_blur_idle_nb);
	...
	case CPUFREQ_GOV_STOP:
		...
		cpufreq_unregister_notifier(
			&cpufreq_notifier_block, CPUFREQ_TRANSITION_NOTIFIER);
		idle_notifier_unregister(&cpufreq_blur_idle_nb);
...
static void cpufreq_blur_nop_timer(unsigned long data)
{
}

static int __init cpufreq_blur_init(void)
{
	unsigned int i;
	struct cpufreq_blur_cpuinfo *pcpu;
	...
	for_each_possible_cpu(i) {
		...
		pcpu->cpu_timer.function = cpufreq_blur_timer;
		pcpu->cpu_timer.data = i;
		init_timer(&pcpu->cpu_slack_timer);
		pcpu->cpu_slack_timer.function = cpufreq_blur_nop_timer;
	...
	mutex_init(&gov_lock);
	speedchange_task =
		kthread_create(cpufreq_blur_speedchange_task, NULL,
			       "cfblur");
	...
	return cpufreq_register_governor(&cpufreq_gov_blur);
}

#ifdef CONFIG_CPU_FREQ_DEFAULT_GOV_BLUR
fs_initcall(cpufreq_blur_init);
#else
module_init(cpufreq_blur_init);
#endif

static void __exit cpufreq_blur_exit(void)
{
	cpufreq_unregister_governor(&cpufreq_gov_blur);
	kthread_stop(speedchange_task);
	put_task_struct(speedchange_task);
}

module_exit(cpufreq_blur_exit);

MODULE_AUTHOR("Mike Chan <mike@android.com>");
MODULE_DESCRIPTION("'cpufreq_blur' - A cpufreq governor for "
